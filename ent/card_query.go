...
func (cq *CardQuery) QueryPlaylist() *PlaylistQuery {
	query := &PlaylistQuery{config: cq.config}
...
func (cq *CardQuery) QueryScans() *CardScanQuery {
	query := &CardScanQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CardQuery) Clone() *CardQuery {
	...
	return &CardQuery{
		...
		withPlaylist: cq.withPlaylist.Clone(),
...
func (cq *CardQuery) WithPlaylist(opts ...func(*PlaylistQuery)) *CardQuery {
	query := &PlaylistQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withPlaylist = query
	return cq
}
...
func (cq *CardQuery) WithScans(opts ...func(*CardScanQuery)) *CardQuery {
	query := &CardScanQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CardQuery) sqlAll(ctx context.Context) ([]*Card, error) {
	...
	if query := cq.withPlaylist; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Card)
		for i := range nodes {
			if fk := nodes[i].playlist_card; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "playlist_card" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := cq.withScans; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Scans = []*CardScan{}
		}
		query.withFKs = true
		query.Where(predicate.CardScan(func(s *sql.Selector) {
			s.Where(sql.InValues(card.ScansColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.card_scan_card
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "card_scan_card" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "card_scan_card" returned %v for node %v`, *fk, n.ID)
			}
